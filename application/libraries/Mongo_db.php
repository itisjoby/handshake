...
Class Mongo_db {
     ...
     */
    public function aggregate_old($collection, $operation, $options = array()) {
        ...
        try {
            $result['result'] = [];
            $documents        = $this->db->{$collection}->aggregate($operation, $options);
            if (isset($documents['cursor']['firstBatch']))
                $result['result'] = $this->decrypt($documents['cursor']['firstBatch']);
    ...
    public function aggregate($collection, $operation, $options = array()) {
        //echo "<pre>";print_r($operation);echo "</pre>"; exit();
        //new stdClass;
        ...
        try {
            ...
            $cursor = MongoCommandCursor::createFromDocument($this->connect, $hash, $r);
            if ($cursor) {
            ...
            $CI = & get_instance();
            if ($CI->uri->segment(1) != 'api' && $CI->uri->segment(3) != 'getMembers') {
                $cursor = $this->decrypt($cursor);
            }
    ...
    function decrypt($arr_data) {
        //print_r($arr_data);exit();
        $CI = & get_instance();
        if ($CI->uri->segment(1) == 'client' && $CI->uri->segment(3) == 'school_dashboard') {
            //print_r($arr_data);exit();
            $client_id = $arr_data['client_id'];
        } else {
        ...
        if ($CI->uri->segment(1) != 'api' && $CI->uri->segment(3) != 'getStudents') {
            $defined_fields = array('admin_username', 'admin_firstname', 'admin_lastname', 'admin_landline', 'admin_email', 'admin_position', 'admin_ward_postcode', 'instructor_username', 'instructor_firstname', 'instructor_lastname', 'instructor_dob', 'instructor_address_one', 'instructor_address_two', 'instructor_city', 'instructor_ward', 'instructor_postcode', 'instructor_mobile', 'instructor_landline', 'instructor_email', 'instructor_gender', 'instructor_notes', 'client_name', 'la_code', 'client_address', 'client_primary_contact_name', 'client_primary_contact_phone', 'client_primary_contact_email', 'client_cost', 'client_support_email', 'client_notes', 'postcode', 'ethnicity', 'name', 'member_number', 'member_forename', 'member_surname', 'member_gender', 'member_email', 'member_address_one', 'member_address_two', 'member_address_three', 'member_ward', 'member_postcode', 'member_home_number', 'member_mobile_number', 'member_office_number', 'member_medical_conditions', 'member_note', 'venue_user_username', 'venue_user_email');
        } else {
            $defined_fields = $defined_fields = array('admin_username', 'admin_firstname', 'admin_lastname', 'admin_landline', 'admin_email', 'admin_position', 'admin_ward_postcode', 'instructor_username', 'instructor_firstname', 'instructor_lastname', 'instructor_dob', 'instructor_address_one', 'instructor_address_two', 'instructor_city', 'instructor_ward', 'instructor_postcode', 'instructor_mobile', 'instructor_landline', 'instructor_email', 'instructor_gender', 'instructor_notes', 'client_name', 'la_code', 'client_address', 'client_primary_contact_name', 'client_primary_contact_phone', 'client_primary_contact_email', 'client_cost', 'client_support_email', 'client_notes', 'postcode', 'ethnicity', 'name', 'member_number', 'member_forename', 'member_surname', 'member_gender', 'member_email', 'member_address_one', 'member_address_two', 'member_address_three', 'member_ward', 'member_postcode', 'member_home_number', 'member_mobile_number', 'member_office_number', 'member_medical_conditions', 'member_note', 'venue_user_username', 'venue_user_email');
        }
        if (!empty($arr_data)) {

            $CI->load->library('encrypt_new');


            $enc_email_key = $CI->encrypt_new->set_email_encryption_key();
            foreach ($arr_data as $key1 => $data1) { //print_r($data1);
                foreach ($data1 as $key => $data) {
                    if (array_key_exists("client_id", $data1)) {
                        $enc_key = $CI->encrypt_new->set_encryption_key($data1['client_id']);
                    } else {
                        $enc_key = $CI->encrypt_new->set_encryption_key($client_id);
                    }
                    //echo $enc_key;exit();
                    if (in_array($key, $defined_fields)) {
                        if ($key == 'client_primary_contact_email' || $key == 'client_support_email' || $key == 'member_email' || $key == 'instructor_email' || $key == 'admin_email' || $key == 'instructor_username' || $key == 'admin_username' || $key == 'venue_user_username' || $key == 'venue_user_email') {
                            $decrypted_data[$key1][$key] = $CI->encrypt_new->decrypt($data, $enc_email_key);
                        } else {
                            if (is_array($data) && !empty($data)) {
                                $data = @$data[0];
                            }
                            if (!is_array($data)) {
                                $decrypted_data[$key1][$key] = $CI->encrypt_new->decrypt($data, $enc_key);
                            }
                            // print_r($decrypted_data);exit();
                        }
                    } else {
                        $decrypted_data[$key1][$key] = $data;
                    }
                }
            }

            $decrypted_data[] = array_pop($decrypted_data);
            //print_r($decrypted_data);exit();
            return $decrypted_data;
        }
    }

    public function aggregate_test($collection, $operation, $options = array()) {


        if (!isset($options['cursor']))
            $options['cursor'] = ['batchSize' => 1];
        ...
        try {
            ini_set('mongo.native_long', false);
            ini_set('mongo.long_as_object', true);
            $cursor           = iterator_to_array($this->db->{$collection}->aggregateCursor($operation, $options));
            $result['result'] = $cursor;
            $this->_clear();
            return $cursor;
        } catch (MongoResultException $e) {
